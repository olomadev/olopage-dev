            ...
            >
              <v-card :loading="$store.getModule('api').getLoading" flat rounded class="card-wrapper hover-card">
                <v-card-title class="d-flex">
                  <v-spacer class="custom-font-size">/{{ item.raw.route }}</v-spacer>
                   <v-chip
                    label
                  ...
                  >
                    ...
                    {{ $t("resources.pages." + item.raw.publishStatus) }}
                  </v-chip>
                  <v-menu>
                ...
                <router-link :to="{ name: 'pages_edit', params: { id: item.raw.id } }">
                  <v-img
...
export default {
  ...
  methods: {
    ...
    async menuItemClick(item, key) {
      if (key === 'duplicate') {
        this.$router.push({ path: "/pages/create", query: { source: item.raw.id }})
      }
      if (key === 'delete') {
        const res = await this.$admin.http({ method: "DELETE", url: "/pages/delete/" + item.raw.id });
